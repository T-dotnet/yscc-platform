...
## Prototype alignment update — longitudinal Report, 19 September 2026

The current local staff **Report** tab opens a selected-care-episode **Care timeline**, not the earlier questionnaire dashboard. It can show dated fictional care periods, medication courses, events, goals, risk-related events and complete compatible K10 responses, with filters and selectable source detail. The Jordan Ellis fixture adds hard-coded visual experiments for design review only. No timeline marker, raw total or visual relationship proves causality, risk, improvement, deterioration or treatment effect; the fixture does not approve a clinical score, data model or production reporting feature. The clinician narrative and visible report-change log remain outside Report. Assessment and History retain the collection, response, review and activity record. See the [current detailed report status](docs/prototype-longitudinal-report-status-2026-09-19.md) and [report contract](docs/05-requirements-and-logic.md#9-clinician-progress-report). Participant access, clinical scoring definitions and broader external reporting scope remain unresolved.

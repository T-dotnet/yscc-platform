...
**Status:** Revised working baseline, prototype alignment revision 16 September 2026; stakeholder decisions remain open
...
**Prototype alignment update, 16 September 2026:** The product owner confirms that every new patient must go through intake (U1/D-25 in document 01). Registration alone cannot start the core assessment. The current [intake/referral contract](docs/05-requirements-and-logic.md#7-intake-and-onward-referral-contract), F-01/F-17, ST-27/ST-28 and AC-23–AC-29 specify the pathway. With fictional browser-local data, the prototype now demonstrates registration, owned intake, proceed/do-not-proceed decisions, assessment-plan gating, separate referral follow-through, purpose-specific consent requests and accept/decline/withdraw history, channel-dependent review handling, and a care-period Progress dashboard with clinician report versions and append-only annotations. Clinical/service parameters, authority and consent policy, review requirements, real delivery, and production access enforcement remain unresolved or unverified; the prototype is not production evidence.
...
| 5. Complete and submit | Answer, use permitted nonresponse options, and receive confirmation when submission succeeds. | Track this assignment, recover failed delivery/save attempts, or record a pause/decline and follow-up. Apply the approved review rule for the recorded channel and assistance context. | Assignment progress, link/session state, response, timestamps, validation, and review-required/not-required decision; FR-08–FR-12, FR-44–FR-45. One response fulfils only its assignment. |
| 6. Clinical review | Receive an explanation of the assessment's progress and next steps. | Where the approved policy requires a separate review, review submitted evidence and outstanding work. Otherwise retain the reason that review is not required. Return to phases 2–5 for additional modules. Record completion or a reason for pausing/closing an incomplete assessment. | Clinical review requirement/state distinct from response and assessment completion; FR-03–FR-08, FR-36–FR-37. |
...
- **FR-08:** Track assignment progress, delivery/link/session state, response submission, the applicable clinical-review requirement/state, and correction history separately, as described below. Derive overdue indicators from due windows and active outstanding work. A submitted response can be pending review, reviewed, or explicitly not require a separate review under an approved mode/assistance rule. Fulfilling one assignment does not complete its assessment; correction does not reopen collection automatically. The current prototype treats clinician entry and supported clinic-tablet completion as review not required and other submitted self-report as pending review; this is sample behaviour awaiting clinical approval.
...
- **FR-22:** Treat each consent as a versioned, purpose-specific request rather than a single record-level setting. Authorised staff select an approved item from a library, set its person and care-episode scope where applicable, confirm decision-maker/authority and eligible channel, then send a traceable request. Preserve the pinned information version, request/delivery history, current status, decision-maker, authority context and effective times. Define care/data collection, contact, and secondary-use purposes with owners; the headspace research-consent extract filter is not a universal care-access rule.
- **FR-23:** Let the authorised participant accept or decline the specific sent request through an approved channel. Check the approved authority, current request status and action-specific consent requirements before collection, sending, or access. Show a blocking reason and owner when requirements are unmet. Sent, declined, expired, cancelled or withdrawn is not accepted consent; declining one purpose must not blanket-block unrelated permitted actions. Sample policy values are allowed in a labelled prototype; production behaviour requires the approved rules.
- **FR-24:** Let the authorised decision-maker withdraw an accepted request for its specific purpose/scope, recording the effective time while preserving the earlier request, delivery and acceptance history. Apply approved rules to affected links, reminders, access, secondary use, and retention. Withdrawal is not an edit back to “not recorded”; any authorised retention/disposal process is separate. Do not assume that withdrawal either deletes all history or permits indefinite retention.
...
| Clinical review | Not required, pending, reviewed; rule/reason, review date and evidence version where applicable | Whether a separate review is required and, when required, a clinician's review of the available evidence. Submission or a not-required result does not automatically complete the assessment. |
...
The current clickable prototype demonstrates the baseline scenarios with synthetic records and clearly labelled sample policy/content, including mandatory intake/referral, consent-request, channel-dependent review and Progress-report interactions added in this revision. These remain design acceptance criteria, not a test-certification claim: browser-local persistence and sample role checks do not prove server-side permissions, approved authority/consent/review policy, external delivery, production persistence, multi-user concurrency, or clinical scoring.
...
| 4. Who answered, who entered, and whether review is required | A clinician transcribes a person's answer. The record identifies both roles accurately and the sample prototype marks clinician entry and supported tablet completion as not requiring a separate clinical review. A permitted joint response remains distinguishable from independent completion. Production review requirements remain an explicit clinical-policy decision. | FR-08, FR-17, FR-21 |
...
| 10. Consent request, decision, and withdrawal | Staff select and send a versioned, purpose-scoped request through an eligible sample channel. The participant can accept or decline that request; sent is not accepted. A later withdrawal preserves request/acceptance history, applies only its approved impact, and does not disable an unrelated permitted purpose. | FR-22–FR-25 |
...
| Which delivery and assistance combinations require a separate clinical review, and what records the not-required decision? | Clinical/content owners | Replace the prototype's sample clinician-entry/supported-tablet rule with an approved instrument- and mode-specific policy. |
...
The staff Report tab leads with a patient-progress dashboard based on questionnaires: dated response history, care-period status counts and respondent/version-scoped answer-change charts. It then presents the clinician report. Charts make comparable answer changes visible but never imply a clinical score, direction, improvement or deterioration without an approved instrument-specific definition. Clinicians can edit the narrative and add append-only care-period annotations. Saved narrative changes to **Summary**, **Clinician interpretation** and **Next steps** must be change logged with before/after wording, editor identity/role, timestamp and retained versions; annotations retain their author/time and report/evidence context without altering answers, reviews or narrative. Cancelled edits and unchanged saves add no entry. The [detailed report contract](docs/05-requirements-and-logic.md#9-clinician-progress-report) defines dashboard limits, section summaries, dated evidence, interpretation/next steps, source traceability, annotations, the report change log and update-needed behaviour. This does not resolve participant report access, clinical scoring definitions or broader external reporting scope.

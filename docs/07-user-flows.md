...
Select person and care period → **Report** → orient to the **Care timeline** date range → use All, Care, Context and conditional K10 filters → select a dated bar or marker → read its factual source detail → open an available source record where needed. Timeline tracks include recorded care periods, medication courses, significant events, goals/functioning, risk-related events and complete compatible K10 responses; only records with valid dates appear. Marker proximity shows timing, not causality, risk, treatment effect or clinical direction. Jordan Ellis exposes additional hard-coded, fictional visual experiments for design review; they are not a reusable reporting contract. Assessment and History remain the collection, response, review and activity trail. The Report hides submitted response history, Clinical notes, clinician-authored narrative, Edit report action and visible report change log. See [the report requirements](05-requirements-and-logic.md#9-clinician-progress-report) and the [current implementation status](prototype-longitudinal-report-status-2026-09-19.md).

...
Version 0.5 · 16 September 2026 · Architecture aligned to the current local prototype
...
└── Person                                                        ST-03
    ...
    └── Selected care episode [persistent context, not repeated clicks]
        ...
        ├── Report                                                ST-29
        │   ├── Progress dashboard and report
        │   ├── Report versions and change log
...
| ST-13 — Questionnaire comparison and details | Selected care-period questionnaire/version, dated response sequence, source/respondent, actual date meanings, answer-level comparison, review history, gaps and valid-comparison indicators.                                                                                                             | Report ST-29; return to the same selected questionnaire and care period.                                                                      | Cross-episode access does not widen authorised data scope; charts/answer changes are not clinical scores or direction.                                                                                                                            |
...
| ST-29 — Progress dashboard and report        | Overall submitted/pending-review/open counts, latest response, separate questionnaire/version selector, question-level Likert charts, qualitative Previous → New changes and answer-level comparison.                                                                                                    | Episode Report tab; questionnaire evidence opens ST-13 without losing care-period context.                                                    | Staff-only in the current scope. The selector scopes dashboard cards and ST-13; Clinical notes and submitted response history remain outside Report in History/Assessment; no participant/guardian access or unsupported clinical interpretation. |
...
## 15. Report and Events tab direction — 16 September 2026
...
The staff person workspace includes **Report**, scoped to the selected care period. It leads with a questionnaire-based dashboard: submitted-response, questionnaire-series, Likert-question and qualitative-change counts plus the latest response date. A separate questionnaire/version selector names the selected instrument and scopes question-level Likert lines, qualitative Previous → New cards and **Questionnaire comparison and details** below it. The selected questionnaire/version group identifies the respondent and care-period date range. Likert charts show dated response points and may show contextual event markers as a different dot style with a hover label; these markers show timing only and do not infer influence, causality, improvement, deterioration or a clinical score. The current implementation can surface available episode events in the chart context, while the Events tab lists recorded care events; whether these sets should be identical is unresolved.

Submitted response history, Clinical notes, the clinician-authored narrative, Edit report action, visible report change log, Latest clinician review and Response & follow-up history are hidden from Report. Assessment and History retain the complete dated collection, review, follow-up and annotation trail. This staff view does not grant participant or guardian access to the report. See [the report requirements](05-requirements-and-logic.md#9-clinician-progress-report).

...
The staff person workspace includes **Report**, scoped to the selected care period. It leads with a questionnaire-based dashboard: submitted-response, questionnaire-series, charted-Likert-question and recorded-contextual-event counts plus the latest response date. A sample **Care coordination context** block follows when configured episode data exists: Risk and status history uses four dated event categories and labels an empty category **Not recorded**; Goals and functioning lists dated structured milestones. It is local prototype data and not a governed risk, treatment or care-plan record. A separate questionnaire/version selector names the selected instrument and scopes question-level Likert lines and **Questionnaire comparison and details** where non-Likert questions apply. The selected questionnaire/version group identifies the respondent and care-period date range. Likert charts show dated response points and may show contextual event markers as a different dot style with a hover label; these markers show timing only. The current implementation also displays a normalised average of valid Likert positions for the selected version. This is a prototype-only descriptive value, not a clinical score, threshold, interpretation or claim of influence, causality, improvement or deterioration. Care-context cards and Likert cards reflow to one column when their available width is too narrow; the comparison preserves each answer's label on narrow screens. The Events tab lists the underlying recorded care events; whether its event set, governance and visibility should match Report remains unresolved. See the [implementation status](prototype-report-and-responsive-status-2026-09-18.md).

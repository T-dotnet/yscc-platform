...
Version 0.5 · 16 September 2026 · Strategy aligned to the current local prototype
...
- For the staff **Report** tab, lead with a questionnaire-based Progress dashboard: preserve the overall counts/latest-response summary, then place a separate questionnaire/version selector above question-level changed-answer charts labelled by respondent/version/date range, with qualitative Previous → New values where applicable. The selector also scopes dated answer-level evidence in the full content column. Hide Submitted response history, **Clinical notes**, the clinician-authored narrative, editing controls and visible report change log. Do not infer a clinical score, direction, improvement or deterioration from answer changes.

...
- **Logic:** Show required/conditional assignments and their actual state, plus provenance and version limits. Record reviewer/date and the specific evidence set reviewed.
...
| AC-04 — Who answered and who entered | A clinician transcribes a person's answer. The record identifies both roles accurately. A permitted joint response remains distinguishable from independent completion. | FR-17, FR-21 |
...
| AC-10 — Purpose-specific blocking | Under a labelled sample policy, missing permission blocks only the affected action and shows its owner/next step. A secondary-use refusal does not automatically disable an unrelated permitted activity. | FR-22–FR-25 |
...
**Product-owner direction, 16 September 2026:** Because questionnaires are complex, the staff **Report** tab should be a report based on the questionnaires, clearly show changes over time, and be editable by the clinician. This applies to the selected care period; participant access remains a separate unresolved scope decision.

- Lead with an understandable report: summary, changes over time, clinician interpretation and next steps. Summarise questionnaire sections before offering item-level detail.
...
- Clinicians can edit the report narrative. Preserve the underlying submitted answers, separate response review status, previous report versions, author/time and evidence snapshot. Corrections to answers use the established response-editing workflow.
- **Change logging — product-owner direction, 16 September 2026:** Log saved changes to **Summary**, **Clinician interpretation** and **Next steps** separately. Each saved version identifies the changed sections, previous and new wording (including removed text), editor identity/role and timestamp. Show the **Report change log** directly below the report, with earlier full versions available. Label the first save as the initial report; cancelled edits, failed saves and unchanged saves add no entry. An evidence-only update can retain a new version with narrative explicitly unchanged. Existing versions remain readable; derive older comparisons only from retained snapshots and do not invent missing metadata.
- New or corrected evidence flags an authored report for review without overwriting clinician text. Saving explicitly reconciles the narrative to current evidence. Prevent saves from stale report/evidence revisions.
...
Acceptance: a clinician can read section summaries, trace findings to dated responses, edit/save/cancel, reload the saved report and change log, inspect before/after wording for each changed section and its editor/time, and recognise newly changed evidence. Removing optional text remains visible in the log; cancelling or saving unchanged wording adds no entry. Different care periods and respondent perspectives remain distinct; a report save neither edits answers nor marks responses reviewed.
...
- Cover questionnaire preparation/replacement, submission and answer corrections; new follow-up collections and their pinned questionnaire/due date; clinical reviews and re-reviews; report edits; participation/contact and data-quality updates; care pause/closure and resulting collection/link changes; linked intake and referral events.
...
Acceptance: plan a follow-up, submit a questionnaire, correct an answer, record/re-record a clinical review, edit the report and record a care event. Each successful action is inspectable with its actor/time and relevant before/after values after reload. Corrections retain originals; person/care-period boundaries, unsuccessful saves and repeated submissions do not create misleading entries.

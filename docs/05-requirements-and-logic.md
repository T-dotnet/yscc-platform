...
- Lead with a patient-progress dashboard: submitted-response, questionnaire-series, charted-Likert-question and recorded-contextual-event counts. When configured event or goal data exists, show the sample **Care coordination context** block before the selector: Risk and status history must distinguish dated recorded events from **Not recorded**, and Goals and functioning must show dated structured milestones rather than an inferred trajectory. Submitted response history is not rendered on Report; complete dated response and follow-up history remains in Assessment and History. A chart must state its respondent, questionnaire version and date range.
- Keep the overall dashboard summary intact, then provide a separate questionnaire/version selector container whose choice scopes the visible Likert cards and answer-level **Questionnaire comparison and details** in the full content column. Hide **Clinical notes** and the clinician-authored narrative from Report; care-period annotations remain available through History. Do not render the Summary, Clinician interpretation, Next steps or report metadata panel on this surface. Summarise questionnaire sections before offering item-level detail.
...
- Only compare clinically compatible measures/versions and the same known respondent. Keep missing dates, different perspectives, unanswered/declined/invalid items, branching and collection context visible. The current local prototype's labelled **Overall questionnaire score** is a normalised average of valid Likert item positions for one selected version; it excludes qualitative answers and must remain visibly non-clinical. Approved clinical scores, direction and interpretation require instrument-specific definitions; answer changes alone do not establish improvement or deterioration.
- Keep dense answer comparison open by default when it orients the task. On narrow screens, preserve the question and dated response labels by converting the comparison to labelled rows, stacking care-context cards before their width becomes unusable, and allowing Likert cards to reflow to one column rather than truncating their questions or answers.

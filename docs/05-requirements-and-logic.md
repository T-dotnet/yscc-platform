...
Version 0.6 · 18 September 2026 · Baseline aligned to the current local prototype
...
The interactive prototype now exercises representative portions of the baseline with fictional records: FR-01 care periods and mandatory intake; FR-06–FR-14 collection points, pinned questionnaire versions, response history, channel-dependent review and corrections; FR-16–FR-21 sample delivery/participant paths; FR-22–FR-25 purpose-specific consent requests and accept/decline/withdraw history; FR-30–FR-38 work queues, audit, intake/referral ownership and episode actions; and the Report and Events tabs described in sections 9–10. Report now includes a sample risk/status event history and dated goal milestones alongside questionnaire evidence. Repeated Likert answers are visualised per question using their labelled ordinal positions, while non-Likert answers remain available through the literal answer-level comparison. The prototype also calculates a normalised 0–100 descriptive average of valid Likert items in the selected version; it excludes qualitative answers and remains neither a clinical score nor an interpretation. Report event markers provide timing context only. The prototype uses browser-local persistence and sample conditions to make these flows testable. See the [implementation status](prototype-report-and-responsive-status-2026-09-18.md) for its exact Report and responsive behaviour.
...
**Current prototype alignment, 19 September 2026:** **Report** is a selected-care-episode longitudinal record. It leads with a shared dated timeline rather than the earlier questionnaire dashboard. The clinician-authored narrative, its edit action and its visible change log remain outside this surface. The stored report model remains in the prototype for now; removal, migration or reuse is a separate data/product decision. This applies to the selected care period; participant access remains a separate unresolved scope decision.

- Render a selected-episode **Care timeline** from dated records only. Care periods and medication courses use bars only when start and end dates are recorded; events, goals and risk-related records use dated markers. Do not infer duration, a missing date or a causal relationship.
- Provide All, Care, Context and conditional K10 track filters. A selectable marker or bar must disclose its factual detail and source/provenance; collection and event source records may open in their existing staff views. Keep an accessible browseable record list as an alternative to the spatial plot.
- Identify the selected care episode, date range, record type, date, source status and, for questionnaire-derived records, instrument/version, respondent, recorder and review state. Undated submitted responses stay out of the timeline and must be stated as unavailable for that view.
- A K10 raw-total line or point is permitted only for a complete, dated, compatible ten-item response with its scoring method visible. The present fixture uses the ABS NHS four-week 1–5 sum and no threshold, diagnosis, clinical direction or treatment-effect claim. This fixture does not approve K10 collection, scoring or interpretation for the service.
- The Jordan Ellis fixture contains extra hard-coded visual experiments for symptoms, programme periods, goals, activity, outcome measures, risk states and medication periods. Treat these only as fictional design-review material: they are not a data model, source-of-record contract, approved display or clinical feature.
- Do not render the earlier questionnaire dashboard, normalised Likert aggregate or **Questionnaire comparison and details** as current Report behaviour. Assessment and History retain the complete collection, response, review and activity trail. Future question-level evidence must preserve compatibility, version, respondent, date and branch limits before any comparison is shown.
...
Acceptance: a clinician can orient to the selected episode, filter the available timeline tracks, inspect a dated item and reach its available source evidence without confusing timing with causality. The Report page contains no submitted response history, clinician-authored narrative, Edit report action or visible report change log. Different care periods and respondent perspectives remain distinct; an annotation neither edits answers nor marks responses reviewed.

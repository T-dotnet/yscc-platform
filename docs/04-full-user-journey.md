...
4. Submitting one response fulfils that assignment only. Clinical review, assessment completion, disposition, and closure are separate actions.
...
| --- | --- | --- | --- |
| Clinician entry | P-04 records clinician ratings or transcribes a respondent's own answers, preserving the distinction. | Save drafts visibly; use pinned item rules; record respondent, recorder, assistance, and observed dates. | Submission confirmation leads back to remaining assessment work, not automatic assessment completion. |
...
| Tablet | P-05 launches participant mode; P-01/P-02 completes independently, assisted, or jointly as approved. | Separate participant and staff context; record assistance; clear local content/session after submit, cancellation, timeout, or reset. | Neutral end screen; authorised clinician checks receipt. Saved draft retention is governed separately from local session clearing. |
...
| Frontstage clinician | Review available responses and their provenance, compare only supported scores, identify outstanding evidence, and decide whether further modules or clarification are needed. |
| System/backstage | Show assessment progress independently of assignment status; record who reviewed which evidence/version and when; keep unfinished work visible. |
...
**Exit evidence:** Review record, outstanding work, completion/pause/incomplete status according to approved criteria.  
...
| Withdrawal or purpose change | P-01/P-03 or staff communicates a permitted decision. | Record purpose/effective time; apply only relevant action/access/retention rules; communicate approved consequences. See F-10. |
...
| Submission and review | One response silently completes the assessment. | AC-02. |
